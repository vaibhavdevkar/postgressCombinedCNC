...
// controller/pmcParameterController.js
const pool = require('../db');
...
exports.getPmcParameterById = async (req, res) => {
  ...
  try {
    const { rows } = await pool.query(
      `SELECT *
         FROM pmc_parameter_master
        WHERE pmc_parameter_id = $1;`,
      [id]
    );
    if (rows.length === 0) {
      return res.status(404).json({ message: 'PMC parameter not found.' });
    }
    res.json(rows[0]);
  } catch (err) {
    console.error('Error fetching PMC parameter:', err);
